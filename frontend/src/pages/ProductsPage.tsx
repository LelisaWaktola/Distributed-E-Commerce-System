...
import { mapProduct, mapCategory } from './HomePage';
...
export default function ProductsPage() {
    ...
    useEffect(() => {
        productApi.getCategories().then((data: any) => {
            const cats = (Array.isArray(data) ? data : []).map(mapCategory);
            setCategories(cats);
        }).catch(() => setCategories([]));
    }, []);
    ...
    useEffect(() => {
        async function fetchProducts() {
            try {
                ...
                let prods: Product[] = (data.products || []).map(mapProduct);

                // Client-side filtering
                if (q) {
                    prods = prods.filter(p => p.name.toLowerCase().includes(q.toLowerCase()));
                }
                if (categorySlug) {
                    const cat = categories.find(c => c.slug === categorySlug);
                    if (cat) prods = prods.filter(p => p.category_id === cat.id);
                }
                ...
                else prods.sort((a, b) => a.name.localeCompare(b.name));
